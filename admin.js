...
function createPlaceRow(place) {
  ...
  const provinceBadge = getProvinceBadge(place.tinh_thanh);
  tdProvince.innerHTML = provinceBadge;
  ...
  tdTime.textContent = '24h';
  tdTime.innerHTML += `<br><small class="text-muted">1 ngày</small>`;
...
function getFormData() {
  return {
    ...
    chi_phi: parseInt(document.getElementById('chi_phi').value),
    thoi_gian: 24, // Cố định 24 giờ (1 ngày)
...
function editPlace(id) {
  ...
  document.getElementById('tinh_thanh').value = place.tinh_thanh;
  document.getElementById('chi_phi').value = place.chi_phi;
  document.getElementById('thoi_gian').value = 24; // Luôn là 24
...
function applyFilters() {
  ...
  if (selectedProvince) {
    filtered = filtered.filter(place => 
      place.tinh_thanh === selectedProvince
    );
  }
...
function bindResetButton() {
  ...
  resetBtn.addEventListener('click', function() {
    const confirmed = confirm('Bạn có chắc muốn reset về dữ liệu mặc định (35 địa điểm)? Tất cả dữ liệu hiện tại sẽ bị xóa!');
...
function resetToDefaultData() {
  ...
  if (typeof resetToDefault === 'function') {
    ...
    showToast('Đã reset về dữ liệu mặc định (35 địa điểm)!', 'info');
...
function resetForm() {
  ...
  document.getElementById('thoi_gian').value = 24; // Reset về 24
  currentEditId = null;
 ...
 * Tạo badge cho tỉnh
 */
function getProvinceBadge(province) {
  const colors = {
    'Tây Ninh': 'primary',
    'Đồng Tháp': 'success',
    'Vĩnh Long': 'info',
  ...
  const color = colors[province] || 'secondary';
  return `<span class="badge bg-${color}">${province}</span>`;
}
...
// Expose functions để có thể gọi từ HTML onclick
