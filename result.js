...
function displaySummary(userInput, results, displayCount) {
  ...
  document.getElementById('summaryTotalTime').textContent = '1 ngày (24 giờ)';
  document.getElementById('summaryRegion').textContent = getProvinceName(userInput.tinh_thanh);
...
function displayPlaceCard(place, rank) {
  ...
  clone.querySelector('.place-region').textContent = place.tinh_thanh;
  clone.querySelector('.place-cost').textContent = formatCurrency(place.chi_phi);
  clone.querySelector('.place-time').textContent = '24 giờ';
  clone.querySelector('.place-days').textContent = '(1 ngày)';
